import React from 'react';
import { Activity, Cpu, LogOut, Server } from 'lucide-react';

const menu = [
  { id: 'it-dashboard', label: 'IT Dashboard', icon: Activity, description: 'Status & workload' },
  { id: 'it-systems', label: 'Systems Health', icon: Server, description: 'Infrastructure view' }
];

const ItSidebar = ({ activeView, setActiveView, onLogout }) => {
  return (
    <aside className="w-72 bg-white border-r border-slate-200 flex flex-col">
      <div className="px-5 py-6 border-b border-slate-200">
        <div className="flex items-center gap-3">
          <div className="p-3 rounded-2xl bg-cyan-50 text-cyan-600">
            <Cpu className="w-6 h-6" />
          </div>
          <div>
            <p className="text-xs uppercase tracking-wide text-slate-500">ANOCAB</p>
            <p className="text-sm font-semibold text-slate-900">IT Department</p>
            <p className="text-xs text-slate-500">Ops & Infrastructure</p>
          </div>
        </div>
      </div>
      <nav className="flex-1 overflow-y-auto px-3 py-5 space-y-2">
        {menu.map((item) => {
          const Icon = item.icon;
          const isActive = activeView === item.id;
          return (
            <button
              key={item.id}
              onClick={() => setActiveView(item.id)}
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-left transition ${
                isActive ? 'bg-cyan-600 text-white shadow-sm' : 'text-slate-600 hover:bg-slate-100'
              }`}
            >
              <Icon className="w-5 h-5" />
              <div>
                <p className="text-sm font-semibold">{item.label}</p>
                <p className={`text-xs ${isActive ? 'text-cyan-100' : 'text-slate-500'}`}>{item.description}</p>
              </div>
            </button>
          );
        })}
      </nav>
      <div className="px-4 py-5 border-t border-slate-200">
        <button
          onClick={onLogout}
          className="w-full flex items-center justify-center gap-2 bg-slate-900 text-white py-2.5 rounded-lg hover:bg-slate-800 transition"
        >
          <LogOut className="w-4 h-4" />
          Logout
        </button>
      </div>
    </aside>
  );
};

export default ItSidebar;

