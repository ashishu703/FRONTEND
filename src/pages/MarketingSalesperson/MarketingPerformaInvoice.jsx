...
const MarketingPerformaInvoice = ({ invoiceData, customer, selectedBranch = 'ANODE' }) => {
  ...
  const discountAmount = invoiceData?.discountAmount != null 
    ? parseFloat(invoiceData.discountAmount) 
    : (subtotal * discountRate) / 100
  const taxableAmount = Math.max(0, subtotal - discountAmount)
  const taxRate = parseFloat(invoiceData?.taxRate != null ? invoiceData.taxRate : 18)
  const cgstRate = 9
  const sgstRate = 9
  const cgstAmount = (taxableAmount * cgstRate) / 100
  const sgstAmount = (taxableAmount * sgstRate) / 100
  const taxAmount = cgstAmount + sgstAmount
  const totalAmount = invoiceData?.total != null 
  ...
  const handlePrint = async () => {
    ...
    const invoiceElement = document.getElementById('marketing-performa-invoice-content')
    ...
    if (printWindow && invoiceElement) {
      ...
      printWindow.document.write(`
        <html>
          <head>
            <title>Performa Invoice - ${invoiceData?.invoiceNumber || 'PI/25-26/458'}</title>
            <style>
    ...
    <div className="max-w-4xl mx-auto bg-white font-sans text-sm">        
      {/* Performa Invoice Content */}
      <div id="marketing-performa-invoice-content" className="p-6">
        {/* Header */}
        ...
        <div className="border border-black mb-4">
          ...
          <div className="grid grid-cols-4 gap-2 p-2 text-xs">
            <div>{invoiceData?.invoiceDate || new Date().toLocaleDateString()}</div>
            <div>{invoiceData?.invoiceNumber || `PI/${new Date().getFullYear().toString().slice(-2)}-${(new Date().getFullYear() + 1).toString().slice(-2)}/${Math.floor(1000 + Math.random() * 9000)}`}</div>
            <div>{invoiceData?.dueDate || new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toLocaleDateString()}</div>
            <div>{invoiceData?.poNumber || `PO-${Math.floor(1000 + Math.random() * 9000)}`}</div>
          </div>
        </div>
        ...
        <div className="border border-black mb-4">
          <div className="grid grid-cols-2 gap-4 p-3 text-xs">
            <div>
              ...
              <p>
                <strong>{invoiceData?.billTo?.business || customer?.business || 'Das Industrial Controls'}</strong>
              </p>
              <p>{invoiceData?.billTo?.address || customer?.address || 'Panvel, Maharashtra, India'}</p>
              <p>
                <strong>PHONE:</strong> {invoiceData?.billTo?.phone || customer?.phone || '7039542259'}
              </p>
              {invoiceData?.billTo?.gstNo || customer?.gstNo ? (
                <p>
                  <strong>GSTIN:</strong> {invoiceData?.billTo?.gstNo || customer?.gstNo}
                </p>
              ) : null}
              <p>
                <strong>State:</strong> {invoiceData?.billTo?.state || customer?.state || 'Maharashtra'}
              </p>
            </div>
            <div>
              <h3 className="font-bold mb-2">SHIP TO:</h3>
              <p>
                <strong>{invoiceData?.shipTo?.business || invoiceData?.billTo?.business || customer?.business || 'Das Industrial Controls'}</strong>
              </p>
              <p>{invoiceData?.shipTo?.address || invoiceData?.billTo?.address || customer?.address || 'Panvel, Maharashtra, India'}</p>
              <p>
                <strong>PHONE:</strong> {invoiceData?.shipTo?.phone || invoiceData?.billTo?.phone || customer?.phone || '7039542259'}
              </p>
              {invoiceData?.shipTo?.gstNo || invoiceData?.billTo?.gstNo || customer?.gstNo ? (
                <p>
                  <strong>GSTIN:</strong> {invoiceData?.shipTo?.gstNo || invoiceData?.billTo?.gstNo || customer?.gstNo}
                </p>
              ) : null}
            </div>
          </div>
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="bg-gray-100">
                ...
                <th className="border border-gray-300 p-1 text-right w-20">Rate</th>
                <th className="border border-gray-300 p-1 text-right w-20">Taxable Value</th>
                <th className="border border-gray-300 p-0.5 text-center w-8 text-[10px] whitespace-nowrap">CGST%</th>
                <th className="border border-gray-300 p-0.5 text-center w-8 text-[10px] whitespace-nowrap">SGST%</th>
              ...
              {invoiceData?.items?.length > 0 ? (
                invoiceData.items.map((item, index) => (
                  <tr key={index}>
                    ...
                    <td className="border border-gray-300 p-1 text-right">{parseFloat(item.rate || 0).toLocaleString('en-IN', {minimumFractionDigits: 2})}</td>
                    ...
                    <td className="border border-gray-300 p-0 text-center text-xs">9%</td>
                    <td className="border border-gray-300 p-0 text-center text-xs">9%</td>
        ...
        <div className="border border-black mb-4">
          <div className="bg-gray-100 p-2 font-bold text-xs">
            <h3>Performa Invoice Terms and Conditions</h3>
          </div>
          <div className="p-3 text-xs space-y-2">
            ...
            <div>
              <h4 className="font-bold">VALIDITY</h4>
              <p>• This Performa Invoice is valid for 7 days from the date of issue.</p>
        ...
        <div className="text-right text-xs">
          ...
          <p className="mb-8">This is computer generated performa invoice no signature required.</p>
          <p className="font-bold">Sales Authorized Signatory</p>
        </div>
        </div>
      </div>
  )
...
export default MarketingPerformaInvoice
